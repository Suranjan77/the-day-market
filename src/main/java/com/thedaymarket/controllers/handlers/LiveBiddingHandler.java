...
public class LiveBiddingHandler {
  ...
  private final ConcurrentHashMap<Long, List<DeferredResult<StreamResponse<BidResponse>>>>
      REQUESTER_BY_AUCTION_ID = new ConcurrentHashMap<>();

  public void notifyAll(Long auctionId, BidResponse bid) {
    var clients = REQUESTER_BY_AUCTION_ID.get(auctionId);
    if (clients != null) {
      List<DeferredResult<StreamResponse<BidResponse>>> completedClients = new ArrayList<>();
      for (var client : clients) {
        client
            .setResult(new StreamResponse<>(StreamStatus.COMPLETED, bid));
  ...
  public void registerClient(Long auctionId, DeferredResult<StreamResponse<BidResponse>> client) {
